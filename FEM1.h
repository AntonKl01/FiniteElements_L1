...
class FEM
{
 public:
  ...
  double                basisFunctionOrder, prob, L, g1, g2, E, A, h, _f;
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  if (basisFunctionOrder == 2) {
    ...
    double devide = (xi0 - xi1) * (xi0 - xi2);
    value = (xi - xi2) / devide + (xi - xi1) / devide;
  }
  ...
  if (basisFunctionOrder == 3) {
    ...
    double devide = (xi0 - xi1) * (xi0 - xi2) * (xi0 - xi3);
    value = (xi - xi2) * (xi - xi3) / devide + (xi - xi1) * (xi - xi3) / devide + (xi - xi1) * (xi - xi2) / devide;
  }
  ...
  for (;elem!=endc; ++elem) {
    ...
    for (unsigned int A = 0; A < dofs_per_elem; A++) {
      for (unsigned int q = 0; q < quadRule; q++) {
        ...
        Flocal[A] += h_e/2.* basis_function(A,quad_points[q]) * quad_weight[q] * _f * x;
      }
    }
...
void FEM<dim>::output_results (){
  ...
  std::ofstream output1("solution_DN_3.vtk");
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem != endc; ++elem) {
    ...
    for(unsigned int q = 0; q < quadRule; q++) {
      ...
      if (prob == 1) {
        u_exact = -_f * x * x * x / (6. * E) + (g2 - g1 + _f * L * L * L /(6. * E))/ L * x + g1;
      }

      else if (prob == 2) {
        u_exact = -_f * x * x *x / (6. * E) + ( (2 * h + _f*L*L) / (2. * E) ) * x + g1;
      }
